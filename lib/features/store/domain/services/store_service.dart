...
class StoreService implements StoreServiceInterface {
  ...
  String filterRestaurantLinkUrl(String slug, Store store) {
    ...
    if (AppConstants.useReactWebsite) {
      if (slug.isNotEmpty) {
        replace =
            '${routes[0]}/$slug?module_id=${store.moduleId}&module_type=${Get.find<SplashController>().module!.moduleType}&store_zone_id=${store.zoneId}&distance=${store.address}';
      } else {
        replace =
            '${routes[0]}/${store.id}?module_id=${store.moduleId}&module_type=${Get.find<SplashController>().module!.moduleType}&store_zone_id=${store.zoneId}&distance=${store.address}';
      }
    } else {
      if (slug.isNotEmpty) {
